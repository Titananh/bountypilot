...
| Skill | `bounty skill score bug-bounty-pilot` | No | Scores skill readiness across validation, temporary bundle verification, release gates, warnings, blockers, next steps, separate `layers.local`/`layers.publish`, and `publicReadiness.missing[].commands` remediation. |
