...
The workflow verifies the release gate, creates an npm tarball, generates `SHA256SUMS.txt`, attests release provenance, and attaches the artifacts to the GitHub release.
It also generates and attaches `bountypilot-sbom.cdx.json` as a CycloneDX SBOM for supply-chain review.
For a standalone skill artifact, run `bounty skill bundle bug-bounty-pilot --output bug-bounty-pilot.skill.zip`; the ZIP includes `MANIFEST.bountypilot.json` with SHA-256 hashes for every skill file.
